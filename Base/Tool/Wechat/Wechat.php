<?php
namespace Base\Tool\Wechat;
/**
 * 微信公众号API快速开发
 * @link https://github.com/binsee/wechat-php-sdk
 * @author dodgepudding
 * @功能修改完善  ：路漫漫 <ahmerry@qq.com>
 * @version 1.0
 *  usage:
 *   $options = array(
...
//        $this->log($postStr);
     ...
     * 获取智能设备绑定事件推送
     */
    public function getRevDevice(){
        if (isset($this->_receive['bind'])){
            $array['bind'] = $this->_receive['Event'];
        }
        if (isset($this->_receive['EventKey'])){
            $array['key'] = $this->_receive['EventKey'];
        }
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	/**
	 * 设置缓存，按需重载
	 * @param string $cachename 缓存名字
	 * @param mixed $value 值
	 * @param int $expired 有效期(s)
	 * @return boolean
     * @todo 该方法可继承后重写
	 */
	...
	public function checkAuth($appid='',$appsecret='',$token=''){
		...
		if ($rs = $this->getCache($this->tokenName))  {
		...
		if ($result) {
            ...
            $this->setCache( $this->tokenName, $this->access_token, $expire );
            return $this->access_token;
        }
	...
	public function createMenu($data){
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_URL_PREFIX.self::MENU_CREATE_URL.'access_token='.$this->access_token,self::json_encode(['button' => $data]));
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode'])) {
...
class ErrorCode {
    ...
    public static $errCode = ['0' => '处理成功', '40001' => '校验签名失败', '40002' => '解析xml失败', '40003' => '计算签名失败', '40004' => '不合法的AESKey', '40005' => '校验AppID失败', '40006' => 'AES加密失败', '40007' => 'AES解密失败', '40008' => '公众平台发送的xml不合法', '40009' => 'Base64编码失败', '40010' => 'Base64解码失败', '40011' => '公众帐号生成回包xml失败'];
