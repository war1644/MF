...
MF
===
    嗯，还是辩证看待下吧。虽然不需要复杂的框架，但是一款简单的框架还是需要吧？MF没有复杂的结构，没有复杂的引用。你只需要在routes里配置路由规则，在Controller里各种new，以及在Model里各种sql就好。
...
* nginx rewrite
    ...
    ```
* Base下工具类根据自己需求保留，用不上的可以删掉，减小框架体积（框架将缩小到100K以内），完全独立。
